...
if "scan_results" in st.session_state:
    ...
    if df_res.empty:
        ...
        # Apply liquidity filter ONLY if user set it > 0
        if min_dollar_vol > 0:
            before = len(df_res)
        ...
        with tab1:
            ...
            if len(longs_div) == 0:
                st.info("No long signals above threshold. Try lowering min score.")
        ...
        with tab5:
            ...
            st.metric("Success Rate", f"{len(df_res)}/{len(df_res)+len(df_err)} "
                     f"({100*len(df_res)/max(1,len(df_res)+len(df_err)):.0f}%)")
            if not df_err.empty:
            ...
            if not df_res.empty:
                st.bar_chart(df_res["Score"].value_counts(bins=10).sort_index())
